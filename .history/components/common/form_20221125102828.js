...
export default function Form({ handleLoadRecords, editRow }) {
  let [date, setDate] = useState( editRow? moment(editRow?.date):  null);
  let [designation, setDesignation] = useState(editRow? editRow?.designation: null);
  ...
  let [client, setClient] = useState(editRow ? editRow?.client : null);
  ...
  return (
    <div className="grid grid-cols-3 gap-5">
      ...
      <Button color="blue" disabled={submitting || !date || !designation || !client || qty===0 || !qty} onClick={() => editRow? editRecord() :  createRecord()}>
