...
export default function RecordsTable({ data, doneLoading, handleEdit, handleDelete}) {
    console.log(data)
  return (
    <div className="flex flex-col items-center justify-center w-full">
      {!doneLoading && !data && <Loader inline active/>}
      
      {doneLoading && data && data.length>0 && <Table compact striped>
        <Table.Header>
          <Table.Row>
          ...
          {data?.map(($, index) => {
            ...
            <Table.Cell>
                <div className="flex flex-row space-x-5">
                   ...
                   <TrashIcon className="h-3 w-3 text-red-700 cursor-pointer" onClick={()=>handleDelete($._id)}/>
                </div>
            </Table.Cell>
          </Table.Row>;
          })}
      ...
      {doneLoading && (!data || data?.length ===0) && 'No data found!'}
    </div>
  );
}
